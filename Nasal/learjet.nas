...
var annunciators_loop = func{
    ...
    if(getprop("engines/engine[0]/turbine") < 44){
    ...
    if(getprop("engines/engine[1]/turbine") < 44){
        Annun.getNode("fuel-psi").setBoolValue(1 * PWR2);
        Annun.getNode("hyd-flow").setBoolValue(1 * PWR2);
        Annun.getNode("hyd-psi").setBoolValue(1 * PWR2);
    }else{
...
var update_systems = func{
    ...
    if(GrWrn ==1){
        if(getprop("engines/engine/n2")<80 or getprop("engines/engine[1]/n2")<80){
            if(getprop("velocities/airspeed-kt") < 124)Ghorn =1;
        }
        if(getprop("/surface-positions/flap-pos-norm") > 0.6)Ghorn =1;
